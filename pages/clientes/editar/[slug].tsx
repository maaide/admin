...
const EditClientPage = () => {
  ...
  const getClientData = async () => {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/clients/${router.asPath.replace('/clientes/editar/', '')}`)
  ...
  const submitForm = async () => {
    ...
    await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/clients/${router.asPath.replace('/clientes/editar/', '')}`, clientData)
      ...
      <LeftMenu>
            ...
            <div className='flex gap-2 ml-auto w-fit'>
              {
                clientData.email === initialEmail
                  ...
                  : <button onClick={submitForm} className='bg-main text-white text-sm rounded-md w-36 h-8'>{submitLoading ? <Spinner2 /> : 'Editar cliente'}</button>
              }
        ...
        <div className='p-6 bg-[#f6f6f7] mb-16 overflow-y-scroll dark:bg-neutral-900' style={{ width: 'calc(100% - 252px)' }}>
          <div className='flex gap-3 mb-4 max-w-1280 m-auto'>
            ...
            <h1 className='text-xl mt-auto mb-auto'>Editar cliente</h1>
          </div>
